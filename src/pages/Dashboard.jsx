...
export default function Dashboard({ role, seances, profs, setPage }) {
  ...
  return (
    <div className="fade-in">
      <h2 style={{ fontSize: 24, fontWeight: 700, color: "#0F172A", marginBottom: 4, fontFamily: "'Playfair Display', serif" }}>
        Bonjour 👋
      </h2>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
        <h3 style={{ fontSize: 16, fontWeight: 700, color: "#0F172A" }}>Prochaines séances</h3>
        <button onClick={() => setPage("seances")}
          style={{ background: "none", border: "none", color: "#1A56DB", fontSize: 13, fontWeight: 600, cursor: "pointer" }}>
          Voir tout →
        </button>
      </div>
          ...
          : upcoming.map(s => (
            <div key={s.id} style={{
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
                <div style={{ background: "#EFF6FF", borderRadius: 8, padding: "8px 12px", textAlign: "center", minWidth: 52 }}>
                  <div style={{ fontSize: 11, color: "#1A56DB", fontWeight: 700 }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: 12, color: "#64748B" }}>{s.heure} · {s.eleve} · {s.niveau}</div>
                </div>
              </div>
      ...
      {role === "parent" && (
        <>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
            <h3 style={{ fontSize: 16, fontWeight: 700, color: "#0F172A" }}>Professeurs recommandés</h3>
            ...
            {profs.filter(p => p.valide).slice(0, 3).map((p, i) => (
              <div key={p.id} style={{ background: "#fff", border: "1px solid #E2E8F0", borderRadius: 12, padding: 18 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
                  <Avatar initials={p.photo} size={38} color={getAvatarColor(i)} />
                  <div>
                    <div style={{ fontWeight: 600, fontSize: 13 }}>{p.prenom} {p.nom}</div>
                    <div style={{ fontSize: 11, color: "#64748B" }}>📍 {p.ville}</div>
                  </div>
                </div>
                <Stars n={p.note} />{" "}
                <span style={{ fontSize: 11, color: "#64748B" }}>({p.avis} avis)</span>
                <div style={{ marginTop: 8, display: "flex", flexWrap: "wrap", gap: 4 }}>
                  {p.matières.slice(0, 2).map(m => <Badge key={m} text={m} />)}
                </div>
                <div style={{ marginTop: 10, fontWeight: 700, color: "#059669", fontSize: 14 }}>
